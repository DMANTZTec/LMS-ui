...
import { ArrowLeft,ArrowLeftCircle,Send, Loader2, X } from 'lucide-react';
...
const selectCls = (hasError) =>
  `h-11 w-full roundded-xl border px-4 text-sm transition outline-none bg-gray-50/50 focus:ring-2 focus:ring-teal-500/20 focus:bg-white text-gray-700
...
const backPage= pathLocation.key !== 'default';
...
console.log("response is: ",response);
  ...
  <>
    ...
    <div className='flex justify-center'>
      ...
      <div className="max-w-[460px] p-0 overflow-hidden border-none rounded-2xl bg-white shadow-2xl  justify-center">
        ...
        <div className=" pl-{150px} bg-[#008080] text-white p-6 text-center space-y-2 relative">
           ...
           <button className="" onClick={() => navigate(-1)}>
            <X className='absolute top-1/5 right-2 border border-[1px] border-[#F42222] bg-white text-[#F42222] h-[16px] w-[16px]'  />
           </button>
           )}
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate className="p-6 space-y-4">
          ...
          <Field label="Current Position" error={errors.currentPosition?.message} labelClassName="text-sm font-semibold text-gray-700">
            <select {...register("currentPosition")} className={selectCls(!!errors.currentPosition)}>
